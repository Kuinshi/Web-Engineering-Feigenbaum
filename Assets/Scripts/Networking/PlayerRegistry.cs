...
{
	/// <summary>
	/// Class completely copied from Photon Fusion's Minigolf Example and adjusted to our needs
	/// </summary>
	public class PlayerRegistry : NetworkBehaviour, INetworkRunnerCallbacks
	{
		...
		public static IEnumerable<PlayerObject> Titan =>  Instance?.Object?.IsValid == true ? Instance.ObjectByRef.Where(kvp => kvp.Value && kvp.Value.IsTitan).Select(kvp => kvp.Value) : Enumerable.Empty<PlayerObject>();
		public static IEnumerable<PlayerObject> Jaeger =>  Instance?.Object?.IsValid == true ? Instance.ObjectByRef.Where(kvp => kvp.Value && !kvp.Value.IsTitan).Select(kvp => kvp.Value) : Enumerable.Empty<PlayerObject>();
		...
		{
			return Everyone.Where(p => match.Invoke(p));
		...
		{
			return Everyone.Count(p => match.Invoke(p));
		...
		public static bool Any(System.Predicate<PlayerObject> match)
		{
			...
			return Everyone.Count(p => match.Invoke(p)) > 0;
		...
		{
			if (match != null) return Everyone.Where(p => match.Invoke(p)).OrderBy(selector);
			return Everyone.OrderBy(selector);
		}
		...
		public static IOrderedEnumerable<PlayerObject> OrderDesc<T>(
			...
			System.Predicate<PlayerObject> match = null) where T : System.IComparable<T>
		{
			if (match != null) return Everyone.Where(p => match.Invoke(p)).OrderByDescending(selector);
			return Everyone.OrderByDescending(selector);
		}

		public static PlayerObject Next(PlayerObject current)
		{
			IEnumerable<PlayerObject> collection = Everyone;
		...
		{
			IEnumerable<PlayerObject> collection = Everyone.Where(p => match.Invoke(p));
			int index = collection.FirstIndex(current);
			if (index == -1) return current;
			int length = collection.Count();
			return collection.ElementAt((int)Mathf.Repeat(index + 1, length));
